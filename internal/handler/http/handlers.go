...
func (h *Handler) ListAccounts(c *gin.Context) {
	...
	var role sharedbiz.Role
	switch roleParam {
	case "", "all":
		role = ""
	case "teacher", "teachers":
		role = sharedbiz.RoleTeacher
	...
	var status sharedbiz.AccountStatus
	switch statusParam {
	case "", "all":
		status = ""
	case string(sharedbiz.AccountStatusActive):
		status = sharedbiz.AccountStatusActive
	case string(sharedbiz.AccountStatusLocked):
		status = sharedbiz.AccountStatusLocked
	case string(sharedbiz.AccountStatusPasswordResetRequired):
		status = sharedbiz.AccountStatusPasswordResetRequired
	default:
		response.Error(c, http.StatusBadRequest, response.CodeInvalidStatus, statusParam)
		return
	}
...
func (h *Handler) ResetAccountPassword(c *gin.Context) {
	...
	if accountID == "" {
		response.Error(c, http.StatusBadRequest, response.CodeAccountIDRequired, nil)
		return
	}
	...
	if err := h.admin.ResetAccountPassword(c.Request.Context(), req.SchoolID, accountID); err != nil {
		h.handleAdminAccountError(c, err)
		return
	}
...
func (h *Handler) UpdateAIAgentSetting(c *gin.Context) {
	...
	setting, err := h.aiSettings.UpdateSetting(c.Request.Context(), service.UpdateAIAgentSettingInput{
		SchoolID:                strings.TrimSpace(req.SchoolID),
		Provider:                aibiz.AIProvider(strings.TrimSpace(strings.ToLower(req.Provider))),
...
func (h *Handler) CreateOssPolicy(c *gin.Context) {
	...
	var status storagebiz.OssPolicyStatus
	switch statusValue {
	case "", string(storagebiz.OssPolicyStatusEnabled):
		status = storagebiz.OssPolicyStatusEnabled
	case string(storagebiz.OssPolicyStatusReadOnly), "readonly":
		status = storagebiz.OssPolicyStatusReadOnly
	case string(storagebiz.OssPolicyStatusDisabled):
		status = storagebiz.OssPolicyStatusDisabled
	default:
		response.Error(c, http.StatusBadRequest, response.CodeInvalidStatus, statusValue)
		return
	}
...
func (h *Handler) UpdateStudentReminder(c *gin.Context) {
	...
	if reminderID == "" {
		response.Error(c, http.StatusBadRequest, response.CodeMissingReminderID, nil)
		return
	}
...
func (h *Handler) GetAssignmentSubmission(c *gin.Context) {
	...
	if assignmentID == "" || submissionID == "" {
		response.Error(c, http.StatusBadRequest, response.CodeMissingAssignmentOrSubmissionID, nil)
		return
	}
...
func parseAssignmentTypes(raw string) []assignmentbiz.AssignmentType {
	if raw == "" {
		return nil
	}
	parts := strings.Split(raw, ",")
	result := make([]assignmentbiz.AssignmentType, 0, len(parts))
	for _, part := range parts {
		...
		case string(assignmentbiz.AssignmentHomework):
			result = append(result, assignmentbiz.AssignmentHomework)
		case string(assignmentbiz.AssignmentExam):
			result = append(result, assignmentbiz.AssignmentExam)
		}
	}
	if len(result) == 0 {
		return nil
...
func studentReminderPayload(item identitybiz.StudentReminder) gin.H {
	payload := gin.H{
		"id":           item.ID,
		"title":        item.Title,
		"description":  item.Description,
...
func parseReminderPriority(raw string) identitybiz.StudentReminderPriority {
	switch strings.ToLower(strings.TrimSpace(raw)) {
	case string(identitybiz.StudentReminderPriorityHigh):
		return identitybiz.StudentReminderPriorityHigh
	default:
		return identitybiz.StudentReminderPriorityNormal
	}
}
...
func (h *Handler) UpdateNote(c *gin.Context) {
	...
	noteID := c.Param("id")
	if noteID == "" {
		response.Error(c, http.StatusBadRequest, response.CodeMissingNoteID, nil)
		return
	}
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrNoteNotFound):
			response.Error(c, http.StatusNotFound, response.CodeNoteNotFound, nil)
...
func (h *Handler) SendMessage(c *gin.Context) {
	...
	if conversationID == "" {
		response.Error(c, http.StatusBadRequest, response.CodeMissingConversationID, nil)
		return
	}
...
func assignmentPayload(assignment assignmentbiz.Assignment, questions []assignmentbiz.AssignmentQuestion, files []storagebiz.File) gin.H {
...
func assignmentQuestionPayload(q assignmentbiz.AssignmentQuestion) gin.H {
	return gin.H{
		"id":            q.ID,
...
func submissionItemsPayload(items []assignmentbiz.SubmissionItem) []gin.H {
	payload := make([]gin.H, 0, len(items))
	for _, item := range items {
		payload = append(payload, gin.H{
			"id":            item.ID,
...
func submissionDetailWithCommentsPayload(detail service.SubmissionDetail, comments []assignmentbiz.SubmissionComment) gin.H {
	payload := submissionDetailPayload(detail)
	payload["comments"] = submissionCommentsPayload(comments)
	return payload
}

func submissionCommentPayload(comment assignmentbiz.SubmissionComment) gin.H {
	return gin.H{
		"id":             comment.ID,
...
func submissionCommentsPayload(comments []assignmentbiz.SubmissionComment) []gin.H {
	payload := make([]gin.H, 0, len(comments))
	for _, comment := range comments {
...
func messagePayload(msg conversationbiz.Message) gin.H {
	return gin.H{
		"id":              msg.ID,
...
func noteCommentPayload(comment notebiz.NoteComment) gin.H {
	return gin.H{
		"id":          comment.ID,
...
func notePayload(note notebiz.Note) gin.H {
