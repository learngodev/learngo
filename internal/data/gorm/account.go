...
// AccountStore implements identitybiz.AccountRepository using GORM.
...
func (s *AccountStore) Create(ctx context.Context, account *identitybiz.Account) error {
	return s.db.WithContext(ctx).Create(account).Error
}

func (s *AccountStore) Update(ctx context.Context, account *identitybiz.Account) error {
	return s.db.WithContext(ctx).Save(account).Error
}

func (s *AccountStore) FindByIdentifier(ctx context.Context, schoolID, identifier string) (*identitybiz.Account, error) {
	var account identitybiz.Account
...
func (s *AccountStore) FindByID(ctx context.Context, id string) (*identitybiz.Account, error) {
	var account identitybiz.Account
...
func (s *AccountStore) ListByIDs(ctx context.Context, ids []string) ([]identitybiz.Account, error) {
	if len(ids) == 0 {
		return nil, nil
	}
	...
	var accounts []identitybiz.Account
...
func (s *AccountStore) ListByRole(
	...
	size int,
	search string,
) ([]identitybiz.Account, int64, error) {
	var (
		accounts []identitybiz.Account
		total    int64
	)
	...
	if role != "" {
		...
		base = base.Where("accounts.role IN ?", []sharedbiz.Role{sharedbiz.RoleTeacher, sharedbiz.RoleStudent})
	}
...
func (s *AccountStore) UpdateStatus(ctx context.Context, accountID, schoolID string, status sharedbiz.AccountStatus) error {
	result := s.db.WithContext(ctx).
		Model(&identitybiz.Account{}).
...
var _ identitybiz.AccountRepository = (*AccountStore)(nil)
