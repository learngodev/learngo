...
type AdminSystemService struct {
	switchRepo    systembiz.SystemSwitchRepository
	parameterRepo systembiz.SystemParameterRepository
	broadcastRepo systembiz.SystemBroadcastRepository
	auditRepo     systembiz.SystemAuditRepository
...
func NewAdminSystemService(
	switches systembiz.SystemSwitchRepository,
	parameters systembiz.SystemParameterRepository,
	broadcasts systembiz.SystemBroadcastRepository,
	audits systembiz.SystemAuditRepository,
) *AdminSystemService {
...
func (s *AdminSystemService) appendAudit(ctx context.Context, schoolID, category, action, operator, detail string) error {
	...
	entry := &systembiz.SystemAuditLog{
		SchoolID:  schoolID,
		Category:  category,
		Action:    action,
		Operator:  operator,
...
func mapSwitchModel(model systembiz.SystemSwitch) AdminSystemSwitch {
...
func mapParameterModel(model systembiz.SystemParameter) AdminSystemParameter {
...
func mapBroadcastModel(model systembiz.SystemBroadcast) AdminSystemBroadcast {
...
func seedSwitchModels() []systembiz.SystemSwitch {
	now := time.Now()
	out := make([]systembiz.SystemSwitch, len(defaultSystemSwitches))
	for i, sw := range defaultSystemSwitches {
		out[i] = systembiz.SystemSwitch{
			ID:               sw.ID,
			Title:            sw.Title,
...
func seedParameterModels() []systembiz.SystemParameter {
	now := time.Now()
	out := make([]systembiz.SystemParameter, len(defaultSystemParameters))
	for i, param := range defaultSystemParameters {
		out[i] = systembiz.SystemParameter{
			ID:               param.ID,
			Key:              param.Key,
			Value:            param.Value,
			Scope:            param.Scope,
...
func seedBroadcastModels() []systembiz.SystemBroadcast {
	now := time.Now()
	out := make([]systembiz.SystemBroadcast, len(defaultSystemBroadcasts))
	for i, b := range defaultSystemBroadcasts {
		out[i] = systembiz.SystemBroadcast{
			ID:             b.ID,
			Title:          b.Title,
...
func seedAuditLogModels() []systembiz.SystemAuditLog {
	out := make([]systembiz.SystemAuditLog, len(defaultSystemAuditLogs))
	for i, log := range defaultSystemAuditLogs {
		out[i] = systembiz.SystemAuditLog{
