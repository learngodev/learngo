...
type AISettingsService struct {
	...
	audits   aibiz.AIAgentSettingAuditRepository
...
func (s *AISettingsService) GetSetting(ctx context.Context, schoolID string) (*aibiz.AIAgentSetting, error) {
...
func (s *AISettingsService) UpdateSetting(ctx context.Context, input UpdateAIAgentSettingInput) (*aibiz.AIAgentSetting, error) {
	// Sanitize API Key
	if input.APIKey != "" {
	...
	case aibiz.AIProviderQwen, aibiz.AIProviderDeepSeek:
	default:
	...
	audit := &aibiz.AIAgentSettingAudit{
		ID:           uuid.NewString(),
		SchoolID:     input.SchoolID,
...
func (s *AISettingsService) ListSettingAudits(ctx context.Context, schoolID string, limit int) ([]aibiz.AIAgentSettingAudit, error) {
	if strings.TrimSpace(schoolID) == "" {
		return nil, errors.New("school_id required")
	}
	if limit <= 0 {
