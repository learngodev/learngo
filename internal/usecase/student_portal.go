...
type StudentPortalService struct {
	...
	reminders   identitybiz.StudentReminderRepository
}
...
func (s *StudentPortalService) GetStudentByAccountID(ctx context.Context, accountID string) (*identitybiz.Student, error) {
...
type CreateStudentReminderInput struct {
	...
	TimeLabel   string
	Route       string
	Priority    identitybiz.StudentReminderPriority
	Icon        string
}
...
func (s *StudentPortalService) ListAssignments(ctx context.Context, accountID string, limit int, courseID string) ([]StudentAssignmentItem, error) {
	...
	if trimmed := strings.TrimSpace(courseID); trimmed != "" {
		filtered := make([]assignmentbiz.Assignment, 0, len(assignments))
...
func (s *StudentPortalService) ListCourseChapters(ctx context.Context, accountID string, courseID string) ([]coursebiz.CourseChapter, error) {
	student, err := s.students.GetByAccountID(ctx, accountID)
	if err != nil {
		return nil, err
	}
...
func (s *StudentPortalService) GetCourseChapter(ctx context.Context, accountID string, courseID string, chapterID string) (*coursebiz.CourseChapter, []storagebiz.File, error) {
	student, err := s.students.GetByAccountID(ctx, accountID)
	if err != nil {
		return nil, nil, err
	}
...
func (s *StudentPortalService) ListExams(ctx context.Context, accountID string, limit int) ([]StudentAssignmentItem, error) {
	...
	assignments, err := s.assignments.ListByClass(ctx, *student.ClassID, limit, []assignmentbiz.AssignmentType{assignmentbiz.AssignmentExam})
	if err != nil {
		return nil, err
	}
	return s.buildAssignmentItems(ctx, student.ID, assignments)
...
func (s *StudentPortalService) ListAgenda(ctx context.Context, accountID string, start, end time.Time, includeAssignments bool) ([]StudentAgendaItem, error) {
	...
	if includeAssignments {
		...
		for _, assignment := range assignmentItems {
			...
			if assignment.Type == assignmentbiz.AssignmentExam {
...
func (s *StudentPortalService) buildScheduleItems(ctx context.Context, sessions []coursebiz.CourseSession) ([]StudentScheduleItem, error) {
	if len(sessions) == 0 {
		return []StudentScheduleItem{}, nil
	}
...
func (s *StudentPortalService) buildAssignmentItems(ctx context.Context, studentID string, assignments []assignmentbiz.Assignment) ([]StudentAssignmentItem, error) {
	...
	submissionMap := make(map[string]assignmentbiz.AssignmentSubmission, len(submissions))
...
func collectCourseIDsFromSessions(sessions []coursebiz.CourseSession) []string {
...
func collectTeacherIDsFromSessions(sessions []coursebiz.CourseSession) []string {
	ids := make([]string, 0, len(sessions))
	for _, session := range sessions {
		if session.TeacherID != nil {
...
func collectSlotIDsFromSessions(sessions []coursebiz.CourseSession) []string {
	ids := make([]string, 0, len(sessions))
	for _, session := range sessions {
		if session.SlotID != "" {
...
func (s *StudentPortalService) ListCustomReminders(ctx context.Context, accountID string) ([]identitybiz.StudentReminder, error) {
...
func (s *StudentPortalService) CreateCustomReminder(ctx context.Context, accountID string, input CreateStudentReminderInput) (*identitybiz.StudentReminder, error) {
	student, err := s.currentStudent(ctx, accountID)
	if err != nil {
		return nil, err
	}
	...
	reminder := &identitybiz.StudentReminder{
		ID:          uuid.NewString(),
...
func (s *StudentPortalService) UpdateCustomReminder(ctx context.Context, accountID, reminderID string, input UpdateStudentReminderInput) (*identitybiz.StudentReminder, error) {
	student, err := s.currentStudent(ctx, accountID)
	if err != nil {
		return nil, err
	}
...
func (s *StudentPortalService) SetReminderCompletion(ctx context.Context, accountID, reminderID string, completed bool) (*identitybiz.StudentReminder, error) {
	student, err := s.currentStudent(ctx, accountID)
	if err != nil {
		return nil, err
	}
...
func (s *StudentPortalService) currentStudent(ctx context.Context, accountID string) (*identitybiz.Student, error) {
	student, err := s.students.GetByAccountID(ctx, accountID)
	if err != nil {
		return nil, err
	}
...
func normalizeReminderPriority(value identitybiz.StudentReminderPriority) identitybiz.StudentReminderPriority {
	switch value {
	case identitybiz.StudentReminderPriorityHigh:
...
func (s *StudentPortalService) GetStudentProfile(ctx context.Context, accountID string) (*identitybiz.Student, error) {
