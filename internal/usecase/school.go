...
type SchoolService struct {
	schools   schoolbiz.SchoolRepository
	timeSlots coursebiz.TimeSlotRepository
}

// NewSchoolService creates a new SchoolService.
func NewSchoolService(schools schoolbiz.SchoolRepository, timeSlots coursebiz.TimeSlotRepository) *SchoolService {
	return &SchoolService{schools: schools, timeSlots: timeSlots}
}

// ListSchools returns all schools.
func (s *SchoolService) ListSchools(ctx context.Context) ([]schoolbiz.School, error) {
	return s.schools.List(ctx)
}

// ListTimeSlots returns all time slots for a school.
func (s *SchoolService) ListTimeSlots(ctx context.Context, schoolID string) ([]coursebiz.TimeSlot, error) {
	return s.timeSlots.List(ctx, schoolID)
}

// GetTimeSlot returns a time slot by ID.
func (s *SchoolService) GetTimeSlot(ctx context.Context, id string) (*coursebiz.TimeSlot, error) {
	return s.timeSlots.FindByID(ctx, id)
}

// CreateTimeSlot creates a new time slot.
func (s *SchoolService) CreateTimeSlot(ctx context.Context, slot *coursebiz.TimeSlot) error {
...
func (s *SchoolService) UpdateTimeSlot(ctx context.Context, slot *coursebiz.TimeSlot) error {
	if err := s.validateTimeSlot(slot); err != nil {
		return err
	}
...
func (s *SchoolService) validateTimeSlot(slot *coursebiz.TimeSlot) error {
