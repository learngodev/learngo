...
type ScheduleService struct {
	timeSlotRepo  coursebiz.TimeSlotRepository
	scheduleRepo  coursebiz.CourseScheduleRepository
	sessionRepo   coursebiz.CourseSessionRepository
...
func (s *ScheduleService) CreateTimeSlot(ctx context.Context, schoolID, name, startTime, endTime string) (*coursebiz.TimeSlot, error) {
	slot := &coursebiz.TimeSlot{
		ID:        uuid.New().String(),
		SchoolID:  schoolID,
		Name:      name,
		StartTime: startTime,
...
func (s *ScheduleService) ListTimeSlots(ctx context.Context, schoolID string) ([]coursebiz.TimeSlot, error) {
	return s.timeSlotRepo.List(ctx, schoolID)
}

// Schedule Rule Management

func (s *ScheduleService) CreateSchedule(ctx context.Context, schoolID, courseID, classID, teacherID, slotID string, dayOfWeek int, location string, classroomID *string, startDate, endDate time.Time) (*coursebiz.CourseSchedule, error) {
	...
	slotMap := make(map[string]coursebiz.TimeSlot, len(availableSlots))
	...
	now := time.Now()
	schedule := &coursebiz.CourseSchedule{
		ID:          uuid.New().String(),
		SchoolID:    schoolID,
		CourseID:    courseID,
		ClassID:     classID,
...
func (s *ScheduleService) ListSchedules(ctx context.Context, schoolID string, courseID string) ([]coursebiz.CourseScheduleDetail, error) {
	return s.scheduleRepo.ListDetailsBySchool(ctx, schoolID, courseID)
}

func (s *ScheduleService) GetScheduleStats(ctx context.Context, schoolID string) (*coursebiz.ScheduleStats, error) {
...
func (s *ScheduleService) GenerateSessions(ctx context.Context, schoolID string, start, end time.Time) error {
	...
	slotMap := make(map[string]coursebiz.TimeSlot)
	...
	for d := start; d.Before(end) || d.Equal(end); d = d.AddDate(0, 0, 1) {
		...
		for _, sched := range schedules {
			...
			session := &coursebiz.CourseSession{
				ID:          uuid.New().String(),
...
func buildSessionsForSchedule(
	...
	sessions := make([]coursebiz.CourseSession, 0)
	for d := startDate; d.Before(endDate) || d.Equal(endDate); d = d.AddDate(0, 0, 1) {
		...
		sessions = append(sessions, coursebiz.CourseSession{
			CourseID:    courseID,
			ClassID:     classID,
			TeacherID:   teacherID,
			SlotID:      slotID,
...
func (s *ScheduleService) ensureNoScheduleConflict(
	ctx context.Context,
	scope string,
	schedules []coursebiz.CourseSchedule,
	dayOfWeek int,
	startDate time.Time,
	endDate time.Time,
	newStartMinute int,
	newEndMinute int,
	slotMap map[string]coursebiz.TimeSlot,
) error {
...
func (s *ScheduleService) ensureNoSessionConflict(ctx context.Context, session coursebiz.CourseSession, excludeSessionID string) error {
...
func (s *ScheduleService) resolveSlot(ctx context.Context, slotID string, slotMap map[string]coursebiz.TimeSlot) (coursebiz.TimeSlot, error) {
	if slot, ok := slotMap[slotID]; ok {
		return slot, nil
	}
	resolved, err := s.timeSlotRepo.FindByID(ctx, slotID)
	if err != nil {
		return coursebiz.TimeSlot{}, err
	}
	if resolved == nil {
		return coursebiz.TimeSlot{}, newScheduleValidationError("未找到对应时间段", "slot_id", "not_found", slotID)
	}
	slotMap[slotID] = *resolved
	return *resolved, nil
}

func slotRangeMinutes(slot coursebiz.TimeSlot) (int, int, error) {
	startH, startM, err := parseTime(slot.StartTime)
	if err != nil {
