...
type AssignmentService struct {
	...
	comments      assignmentbiz.SubmissionCommentRepository
...
type CreateAssignmentInput struct {
	...
	TeacherID     string
	ClassID       string
	Type          assignmentbiz.AssignmentType
	Title         string
	Description   string
	StartAt       *time.Time
	DueAt         *time.Time
...
type QuestionInput struct {
	Type       assignmentbiz.QuestionType
	Prompt     string
	Options    string
	Answer     string
	Score      float64
...
func (s *AssignmentService) CreateAssignment(ctx context.Context, input CreateAssignmentInput) (*assignmentbiz.Assignment, error) {
	...
	questions := make([]assignmentbiz.AssignmentQuestion, 0, len(input.Questions))
	for _, q := range input.Questions {
		questions = append(questions, assignmentbiz.AssignmentQuestion{
			ID:         uuid.NewString(),
			Type:       q.Type,
			Prompt:     q.Prompt,
			Options:    q.Options,
	...
	attachments := make([]assignmentbiz.AssignmentAttachment, 0, len(input.Attachments))
	for _, fileID := range input.Attachments {
		attachments = append(attachments, assignmentbiz.AssignmentAttachment{
			ID:           uuid.NewString(),
	...
	go func() {
		...
		if err == nil {
			for _, student := range students {
				s.notifications.Create(bgCtx, student.AccountID, "New Assignment: "+input.Title, "A new assignment has been published.", notificationbiz.NotificationTypeAssignment, assignment.ID)
			}
		}
	}()
...
func (s *AssignmentService) Submit(ctx context.Context, input SubmitAssignmentInput) error {
	...
	items := make([]assignmentbiz.SubmissionItem, 0, len(input.Answers))
	for _, ans := range input.Answers {
		items = append(items, assignmentbiz.SubmissionItem{
			ID:         uuid.NewString(),
...
func (s *AssignmentService) GetAssignment(ctx context.Context, assignmentID string) (*assignmentbiz.Assignment, []assignmentbiz.AssignmentQuestion, []storagebiz.File, error) {
	if assignmentID == "" {
...
func (s *AssignmentService) ListAssignmentSubmissions(ctx context.Context, assignmentID string) ([]SubmissionDetail, error) {
	...
	itemsBySubmission := make(map[string][]assignmentbiz.SubmissionItem, len(subs))
...
func (s *AssignmentService) GetSubmissionForStudent(ctx context.Context, assignmentID, studentID string) (*SubmissionDetail, []assignmentbiz.SubmissionComment, error) {
...
func (s *AssignmentService) GetSubmissionForTeacher(ctx context.Context, teacherID, assignmentID, submissionID string) (*SubmissionDetail, []assignmentbiz.SubmissionComment, error) {
...
func (s *AssignmentService) GradeSubmission(ctx context.Context, teacherID string, input GradeSubmissionInput) (*SubmissionDetail, []assignmentbiz.SubmissionComment, error) {
	if teacherID == "" {
	...
	itemByID := make(map[string]assignmentbiz.SubmissionItem, len(items))
	for _, item := range items {
		itemByID[item.ID] = item
	}

	updates := make([]assignmentbiz.SubmissionItem, 0, len(input.ItemScores))
	...
	if input.Comment != nil && strings.TrimSpace(input.Comment.Content) != "" {
		...
		comment := &assignmentbiz.SubmissionComment{
			ID:           uuid.NewString(),
			SubmissionID: submission.ID,
			AuthorID:     input.AccountID,
			AuthorRole:   sharedbiz.RoleTeacher,
...
func (s *AssignmentService) ReturnSubmission(ctx context.Context, teacherID string, input ReturnSubmissionInput) (*SubmissionDetail, []assignmentbiz.SubmissionComment, error) {
	if teacherID == "" {
		return nil, nil, errors.New("teacher id required")
	}
	if input.AssignmentID == "" || input.SubmissionID == "" {
	...
	if err := s.submissions.UpdateGrades(ctx, submission, []assignmentbiz.SubmissionItem{}); err != nil {
		return nil, nil, err
	}
...
func (s *AssignmentService) UpdateAssignment(ctx context.Context, input UpdateAssignmentInput) (*assignmentbiz.Assignment, error) {
...
func mergeItems(original []assignmentbiz.SubmissionItem, updates []assignmentbiz.SubmissionItem) []assignmentbiz.SubmissionItem {
	if len(updates) == 0 {
		return original
	}
	updated := make(map[string]assignmentbiz.SubmissionItem, len(updates))
	for _, item := range updates {
		updated[item.ID] = item
	}
	result := make([]assignmentbiz.SubmissionItem, 0, len(original))
