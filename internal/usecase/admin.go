...
type AdminService struct {
	...
	departments  schoolbiz.DepartmentRepository
	classes      schoolbiz.ClassRepository
	schedules    coursebiz.CourseScheduleRepository
	teacherLinks identitybiz.TeacherStudentRepository
	aiModel      AIChatModel
	aiSettings   aibiz.AIAgentSettingRepository
}
...
func NewAdminService(
	acc identitybiz.AccountRepository,
	teachers identitybiz.TeacherRepository,
	students identitybiz.StudentRepository,
	departments schoolbiz.DepartmentRepository,
	classes schoolbiz.ClassRepository,
	schedules coursebiz.CourseScheduleRepository,
	links identitybiz.TeacherStudentRepository,
	aiModel AIChatModel,
	aiSettings aibiz.AIAgentSettingRepository,
) *AdminService {
	return &AdminService{
...
func (s *AdminService) CreateTeacher(ctx context.Context, input CreateTeacherInput) (*identitybiz.Teacher, error) {
	if input.DefaultPwd == "" {
...
func (s *AdminService) CreateStudent(ctx context.Context, input CreateStudentInput) (*identitybiz.Student, error) {
	if input.DefaultPwd == "" {
		return nil, errors.New("default password required")
	}
...
type ListAccountsOptions struct {
	SchoolID        string
	Role            sharedbiz.Role
	Status          sharedbiz.AccountStatus
	DepartmentID    string
...
func (s *AdminService) ListAccounts(ctx context.Context, opts ListAccountsOptions) ([]AdminAccountSummary, int64, error) {
	...
	if opts.Role != "" && opts.Role != sharedbiz.RoleTeacher && opts.Role != sharedbiz.RoleStudent {
...
func (s *AdminService) ResetAccountPassword(ctx context.Context, schoolID, accountID string) error {
	...
	if account.Status == sharedbiz.AccountStatusPasswordResetRequired {
		return ErrAdminPasswordResetPending
	}

	if err := s.accounts.UpdateStatus(ctx, accountID, schoolID, sharedbiz.AccountStatusPasswordResetRequired); err != nil {
...
func (s *AdminService) LockAccount(ctx context.Context, schoolID, accountID string) error {
	...
	if account.Status == sharedbiz.AccountStatusLocked {
		return ErrAdminAccountAlreadyLocked
	}

	if err := s.accounts.UpdateStatus(ctx, accountID, schoolID, sharedbiz.AccountStatusLocked); err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrAdminAccountNotFound
		}
		return err
...
func (s *AdminService) UnlockAccount(ctx context.Context, schoolID, accountID string) error {
	...
	if err := s.accounts.UpdateStatus(ctx, accountID, schoolID, sharedbiz.AccountStatusActive); err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrAdminAccountNotFound
		}
		return err
...
func (s *AdminService) ensureManageableAccount(ctx context.Context, schoolID, accountID string) (*identitybiz.Account, error) {
	if accountID == "" {
	...
	if account.Role != sharedbiz.RoleTeacher && account.Role != sharedbiz.RoleStudent {
...
func (s *AdminService) CreateDepartment(ctx context.Context, schoolID, name string) (*schoolbiz.Department, error) {
	department := &schoolbiz.Department{
		ID:       uuid.NewString(),
		SchoolID: schoolID,
		Name:     name,
	}
...
func (s *AdminService) CreateClass(ctx context.Context, schoolID, departmentID, name string) (*schoolbiz.Class, error) {
	class := &schoolbiz.Class{
		ID:           uuid.NewString(),
		SchoolID:     schoolID,
...
func (s *AdminService) ListDepartments(ctx context.Context, schoolID string) ([]schoolbiz.Department, error) {
	if schoolID == "" {
		return nil, errors.New("school_id required")
	}
...
func (s *AdminService) ListClasses(ctx context.Context, schoolID, departmentID string) ([]schoolbiz.Class, error) {
	if schoolID == "" {
		return nil, errors.New("school_id required")
	}
	if departmentID == "" {
...
func (s *AdminService) UpdateDepartment(ctx context.Context, schoolID, departmentID, name string) (*schoolbiz.Department, error) {
...
func (s *AdminService) UpdateClass(ctx context.Context, schoolID, classID, name string) (*schoolbiz.Class, error) {
	trimmed := strings.TrimSpace(name)
	if schoolID == "" || classID == "" {
...
func (s *AdminService) UpdateAccount(ctx context.Context, input UpdateAccountInput) error {
	...
	if account.Role == sharedbiz.RoleStudent {
	...
	} else if account.Role == sharedbiz.RoleTeacher {
...
func (s *AdminService) resolveAccount(ctx context.Context, schoolID, number, name string) (*identitybiz.Account, error) {
	if number != "" {
...
func (s *AdminService) resolveClassID(ctx context.Context, schoolID, deptName, className string) (string, error) {
	...
	if targetDeptID != "" {
		candidateDepts = []schoolbiz.Department{}
		for _, d := range depts {
