...
type AuthService struct {
	...
	resetTokens identitybiz.PasswordResetTokenRepository
...
func NewAuthService(accounts identitybiz.AccountRepository, tokens identitybiz.PasswordResetTokenRepository, cfg config.AppConfig) *AuthService {
...
func (s *AuthService) Login(ctx context.Context, schoolID, identifier, password string) (string, string, *identitybiz.Account, error) {
	...
	case sharedbiz.AccountStatusLocked:
		return "", "", nil, ErrAccountLocked
	case sharedbiz.AccountStatusPasswordResetRequired:
...
func (s *AuthService) RefreshTokens(ctx context.Context, refreshToken string) (string, string, *identitybiz.Account, error) {
...
func (s *AuthService) GetAccount(ctx context.Context, accountID string) (*identitybiz.Account, error) {
...
func (s *AuthService) RequestPasswordReset(ctx context.Context, schoolID, identifier string) (string, time.Time, error) {
	...
	token := &identitybiz.PasswordResetToken{
		ID:        uuid.NewString(),
...
func (s *AuthService) ResetPassword(ctx context.Context, schoolID, identifier, token, newPassword string) error {
	...
	if err := s.accounts.UpdateStatus(ctx, account.ID, schoolID, sharedbiz.AccountStatusActive); err != nil {
		return err
	}
