...
type ConversationService struct {
	conversations conversationbiz.ConversationRepository
	messages      conversationbiz.MessageRepository
	receipts      conversationbiz.MessageReceiptRepository
	accounts      identitybiz.AccountRepository
}

// NewConversationService constructs a ConversationService instance.
func NewConversationService(conversations conversationbiz.ConversationRepository, messages conversationbiz.MessageRepository, receipts conversationbiz.MessageReceiptRepository, accounts identitybiz.AccountRepository) *ConversationService {
...
type ConversationSummary struct {
	Conversation   conversationbiz.Conversation
	Members        []conversationbiz.ConversationMember
	MemberProfiles map[string]identitybiz.Account
	LastMessage    *conversationbiz.Message
	UnreadCount    int64
}
...
func (s *ConversationService) CreateDirectConversation(ctx context.Context, initiatorID, participantID string) (*ConversationSummary, error) {
	...
	if err == nil {
		...
		var last *conversationbiz.Message
	...
	now := time.Now()
	conv = &conversationbiz.Conversation{
		ID:        uuid.NewString(),
		Type:      "direct",
...
func (s *ConversationService) ListConversations(ctx context.Context, accountID string) ([]ConversationSummary, error) {
	...
	for _, conv := range convs {
		...
		profiles := make(map[string]identitybiz.Account)
...
func (s *ConversationService) SendMessage(ctx context.Context, senderID string, input SendMessageInput) (*conversationbiz.Message, error) {
	...
	receipts := make([]conversationbiz.MessageReceipt, 0, len(members))
	for _, member := range members {
		if member.AccountID == senderID {
			continue
		}
		receipts = append(receipts, conversationbiz.MessageReceipt{
			ID:        uuid.NewString(),
			MessageID: msg.ID,
...
func (s *ConversationService) ListMessages(ctx context.Context, accountID, conversationID string, limit int, beforeID string) ([]conversationbiz.Message, error) {
