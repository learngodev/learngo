...
type AdminOssService struct {
	credentials storagebiz.OssCredentialRepository
	policies    storagebiz.OssPolicyRepository
	audits      storagebiz.OssAuditRepository
...
type AdminOssPolicyView struct {
	...
	Status        storagebiz.OssPolicyStatus `json:"status"`
...
type UpdateOssPolicyStatusInput struct {
	SchoolID   string
	PolicyID   string
	Status     storagebiz.OssPolicyStatus
	OperatorID string
}
...
func (s *AdminOssService) CreateCredential(ctx context.Context, input CreateOssCredentialInput) (*AdminOssCredentialView, error) {
	...
	credential := &storagebiz.OssCredential{
		ID:                   uuid.NewString(),
		SchoolID:             schoolID,
		Name:                 name,
		Endpoint:             endpoint,
...
func (s *AdminOssService) CreatePolicy(ctx context.Context, input CreateOssPolicyInput) (*AdminOssPolicyView, error) {
	...
	switch status {
	case storagebiz.OssPolicyStatusEnabled, storagebiz.OssPolicyStatusReadOnly, storagebiz.OssPolicyStatusDisabled:
	default:
		return nil, errors.New("invalid status")
	}

	now := time.Now()
	policy := &storagebiz.OssPolicy{
		ID:            uuid.NewString(),
		SchoolID:      schoolID,
		Name:          name,
...
func toOssCredentialView(cred *storagebiz.OssCredential) AdminOssCredentialView {
	var masked string
...
func toOssPolicyView(policy *storagebiz.OssPolicy) AdminOssPolicyView {
